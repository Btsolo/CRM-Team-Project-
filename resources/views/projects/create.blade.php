...
<form action="{{ route('projects.store') }}" method="POST">
    ...
    <div class="mt-2">
        ...
        <x-text-input id="start_date" class="block mt-1 w-full" type="datetime-local" name="start_date" :value="old('start_date')" required autocomplete="start_date" />
        <x-input-error :messages="$errors->get('start_date')" class="mt-2" />
    </div>

    <div class="mt-2">
        <x-input-label for="end_date" :value="__('EndDate')" />
        <x-text-input name="end_date" id="end_date" class="mt-1 w-full block" type="datetime-local" :value="old('end_date')" autocomplete="end_date" />
        <x-input-error :messages="$errors->get('end_date')" class="mt-2" />
    </div>

    <div class="mt-2">
        <x-input-label for="budget" :value="__('Budget')" />
        <x-text-input name="budget" class="mt-1 w-full block" type="text" :value="old('budget')" required autocomplete="budget" placeholder="Currency is in dollar and should be number only" />
        <x-input-error :messages="$errors->get('budget')" class="mt-2" />
    </div>

    <div class="mt-2">
        <x-input-label for="actual_cost" :value="__('ActualCost')" />
        <x-text-input name="actual_cost" id="actual_cost" type="text" :value="old('actual_cost')" class="mt-1 w-full block" placeholder="Currency is in dollar and should be number only" />
        <x-input-error :messages="$errors->get('actual_cost')" class="mt-2" />
    </div>

    <div class="mt-2">
        <x-input-label for='customer_id' :value="__('SelectCustomer')" />
    ...
    <div class="mt-2">
        ...
        <textarea name="notes" id="notes" {{ old('notes') }} class="block mt-1 w-full rounded shadow-sm focus:ring-indigo-500"></textarea>
