...
<form method="POST" action="{{ route('tasks.store') }}">
    ...
    <div class="mt-4">
        ...
        <textarea name="description" id="description"  class="w-full block rounded-md shadow-sm border-gray-300">{{old('description',$task->description)}}</textarea>
    ...
    <div class="mt-4">
        ...
        <select name="status" id="status" class = "mt-1 w-full block rounded-md shadow-sm border-gray-300">
            ...
            @foreach (\App\Enum\TaskStatus::cases() as $status )
                <option value="{{ $status->value }}" {{old('status',$task->status) == $status->value ? 'selected' : ''}} >{{ ucfirst($status->value) }}</option>
            @endforeach
        </select>
    ...
    <div class="mt-4">
        ...
        <select name="priority" id="priority" class = "mt-1 w-full block rounded-md shadow-sm border-gray-300">
            ...
            @foreach (\App\Enum\TaskPriority::cases() as $priority )
                <option value="{{ $priority->value }}" {{old('priority',$task->priority) == $priority->value ? 'selected' : ''}} >{{ ucfirst($priority->value) }}</option>
            @endforeach
        </select>
        ...
        <div class="mt-4">
            ...
            <select name="customer_id" id="customer_id" class = "mt-1 w-full block rounded-md shadow-sm border-gray-300">
                ...
                @foreach ($customers as $customer )
                    <option value="{{ $customer->id }}" {{old('customer_id',$task->customer_id) == $customer->id ? 'selected' : ''}} >{{ ucfirst($customer->first_name. ' '. $customer->last_name) }}</option>
                @endforeach
            </select>
